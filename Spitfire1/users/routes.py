...
from Spitfire1 import db, bcrypt
from Spitfire1.models import User, Post
from Spitfire1.users.forms import RegistrationForm, LoginForm, UpdateAccountForm, RequestResetForm, ResetPasswordForm
from Spitfire1.users.utils import save_picture, send_reset_email
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, password=hashed_password, group_id=0)
        db.session.add(user)
        db.session.commit()
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Reset Email Sent! Check your spam folder, if it isnt in your inbox.', 'info')
        return redirect(url_for('users.login'))
