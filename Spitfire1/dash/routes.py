...
def taskview(group_id):
    ...
    tasks = Task.query.filter_by(group_id=group_id.id, completed=False).order_by(Task.date_posted.desc())
    ammount = User.query.filter_by(group_id=current_user.group_id)
    return render_template('dashboard.html', group_id=group_id, tasks=tasks, ammount=ammount, title="Dashboard")
...
def taskviewcompleted(group_id):
    ...
    tasks = Task.query.filter_by(group_id=group_id.id, completed=True).order_by(Task.date_posted.desc())
    ammount = User.query.filter_by(group_id=current_user.group_id)
    return render_template('dashboard2.html', group_id=group_id, tasks=tasks, ammount=ammount, title="Dashboard")
...
def new_task(group_id):
    ...
    if form.validate_on_submit():
        task = Task(TaskName=form.title.data, content=form.content.data, author=current_user, group_id=current_user.group_id, completed=False)
        db.session.add(task)
        db.session.commit()
        flash('Task Created.', 'success')
        return redirect(url_for('dash.taskview', group_id=current_user.group_id))
    return render_template('createtask.html', title="Create a new task", form=form, legend='New Task')

@dash.route('/dashboard/group/<int:group_id>/task/<int:task_id>', methods=['POST', 'GET'])
@login_required
...
@dash.route('/dashboard/group/<int:group_id>/completed/task/<int:task_id>', methods=['POST', 'GET'])
@login_required
...
@dash.route('/dashboard/group/<int:group_id>/task/<int:task_id>/setascompleted', methods=['POST', 'GET'])
@login_required
...
@dash.route('/dashboard/group/<int:group_id>/task/<int:task_id>/setasuncompleted', methods=['POST', 'GET'])
@login_required
...
@dash.route('/dashboard/group/<int:group_id>/task/<int:task_id>/edit', methods=['GET', 'POST'])
@login_required
def update_task(task_id, group_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('dash.taskview', task_id=task.id, group_id=current_user.group_id))
    elif request.method == 'GET':
        form.title.data = task.TaskName
        form.content.data = task.content
    return render_template('edittask.html', title="Edit Task", form=form, legend='Edit Post')

@dash.route('/dashboard/group/<int:group_id>/task/<int:task_id>/delete', methods=['POST'])
@login_required
